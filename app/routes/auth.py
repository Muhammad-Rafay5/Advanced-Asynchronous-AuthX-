...
from app.schemas.user import UserCreate, UserRegistrationResponse, PasswordResetRequest, PasswordResetConfirm
...
@router.post("/forgot-password", response_model=StandardActionResponse)
@limiter.limit("3/minute")
async def forgot_password(request: Request, reset_request: PasswordResetRequest, db: AsyncSession = Depends(get_db)):
    result = await auth_service.request_password_reset(db, reset_request.email)
    return StandardActionResponse(detail=result["message"])


@router.post("/reset-password", response_model=StandardActionResponse)
@limiter.limit("3/minute")
async def reset_password(request: Request, reset_confirm: PasswordResetConfirm, db: AsyncSession = Depends(get_db)):
    result = await auth_service.reset_password(db, reset_confirm.token, reset_confirm.new_password)
    return StandardActionResponse(detail=result["message"])
